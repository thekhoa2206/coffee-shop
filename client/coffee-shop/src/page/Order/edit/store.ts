...
import { CustomerRequest, OrderRequest, ProductItemRequest } from "../create/CreateOrder.types";
...
export const useOrderStore = create<OrderStore>((set) => ({
    ...
    addProductLineItem: () =>
        set((prev) => {
            const minValue = Math.min(...prev.lineItems.map((p) => p.id));
            return ({
                ...prev,
                lineItems: [...(prev.lineItems || []), {
                    id: minValue > 0 ? minValue + 1 : minValue - 1,
                }],
            })
        }),
