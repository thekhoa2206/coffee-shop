...
import { TopShipperProps } from "./TopShipper.types";
...
import TopItemComponent from "../TopShipperStore/TopItemComponent/TopItem.component";

const TopShipper = (props: TopShipperProps) => {
    ...
    const initData = async () => {
        let res = await OrdersService.analyticsTopStore(filterTopShipper);
        if (res.data) {
    ...
    return <Fragment>
        <Box className={classes.root} borderRadius="3px" style={{ marginTop: 30, marginBottom: 50 }}>
            <Box className="box-title">
                <Typography variant="h6">TOP DOANH THU CỬA HÀNG</Typography>
            ...
            {loading ? (
                ...
                <Box className={classes.topSaleList}>
                    {listTopShipper && listTopShipper.length > 0 ? (
                        listTopShipper.map(
                            (item, index) =>
                                item && <TopItemComponent item={item} index={index + 1} key={index} isStore={true}/>
                        )
                    ) : (
...
export default connect(mapStateToProps)(withStyles(styles)(TopShipper));